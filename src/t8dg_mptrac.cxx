...
t8dg_mptrac_box::t8dg_mptrac_box (double center_lon, double center_lat, double center_p, double extend_lon_deg, double extend_lat_deg,
                                  double extend_p_km)
:                   box_center
{
center_lon, center_lat, center_p}, box_extend {
extend_lon_deg, extend_lat_deg, extend_p_km}
...
int
t8dg_mptrac_box::lon_lat_pressure_is_in (const double lon, const double lat, const double pressure) const
{
...
double
t8dg_mptrac_box::dist_from_center (const double lon, const double lat, const double pressure) const
{
  const double        pressure_in_km = Z (pressure);
...
double
t8dg_mptrac_box::dist_from_center_maxnorm (const double lon, const double lat, const double pressure) const
{
  const double        pressure_in_km = Z (pressure);
  double              coords_in_km[3], coords_center_in_km[3];
  double              diff[3];
...
double
t8dg_mptrac_box::horiz_dist_from_center_maxnorm (const double lon, const double lat, const double pressure) const
{
  const double        pressure_in_km = Z (pressure);
  double              coords_in_km[3], coords_center_in_km[3];
  double              diff[2];
...
double
t8dg_mptrac_box::horiz_dist_from_center_maxnorm_deg (const double lon, const double lat) const
{
  double              diff[2];
...
double
t8dg_mptrac_box::vert_dist_from_center_maxnorm (const double pressure) const
{
  const double        pressure_in_km = Z (pressure);
...
void
t8dg_mptrac_flux_data::initialize (const struct t8dg_linear_advection_diffusion_problem *problem)
{
...
void
t8dg_mptrac_flux_data::before_first_call_on_element (t8_forest_t forest, t8_locidx_t itree, t8_locidx_t element_in_tree)
{
...
bool
t8dg_mptrac_flux_data::is_current_element_at_pole () const
{
...
void
t8dg_mptrac_flux_data::start_new_time_step (const struct t8dg_linear_advection_diffusion_problem *problem)
{
  ...
  t8dg_debugf ("Mptrac. time = %f  hours_since = %f, diff = %f\n", physical_time_s, hours_since_last_file_read, delta_t_hours);
...
const double
t8dg_mptrac_flux_data::get_time () const
{
  return physical_time_s;
}

const t8_mptrac_context_t *
t8dg_mptrac_flux_data::get_context () const
{
  return context;
}

int
t8dg_mptrac_flux_data::point_is_in_box (const double x[3]) const
{
...
double
t8dg_mptrac_flux_data::point_distance_from_box_center (const double x[3]) const
{
  double              lon, lat, pressure;
...
double
t8dg_mptrac_flux_data::point_distance_from_box_center_maxnorm (const double x[3]) const
{
  double              lon, lat, pressure;
...
void
t8dg_mptrac_flux_data::point_max_vert_and_horiz_distance_from_box (const double x[3], double *vert_dist, double *horiz_dist) const
{
  double              lon, lat, pressure;
...
t8dg_mptrac_flow_3D_fn (double x_vec[3], double flux_vec[3], double t, const t8dg_flux_data_base * flux_data)
{
  ...
  int                 ci[3];
  double              cw[3];
  const met_t        *meteo1 = (const met_t *) t8_shmem_array_index (mptrac_context->mptrac_meteo, 0);
  const met_t        *meteo2 = (const met_t *) t8_shmem_array_index (mptrac_context->mptrac_meteo, 1);
  ...
  intpol_met_time_3d (meteo1_noconst, meteo1_noconst->u,
                      meteo2_noconst, meteo2_noconst->u, physical_time_s, pressure, lon, lat, flux_vec, ci, cw, 1);
  ...
  if (!mptrac_flux_data->is_current_element_at_top_or_bottom ()) {
    ...
    intpol_met_time_3d (meteo1_noconst, meteo1_noconst->w, meteo2_noconst, meteo2_noconst->w, physical_time_s, pressure, lon, lat, flux_vec + 2, ci, cw, 0);    /* 0 here since we can reuse the interpolation weights */
  }
  else {
    flux_vec[2] = 0;
  }
   ...
   * gradient in hPa/h to km/h analytically.
   * This is not the same as converting hPa to km.
   *
   * P -> pressure, function P(Z)
   * Z -> Scale z with Z_min, Z_max
   * z -> x_vec[2] \in {0,1}
   * We know dP/dt (Input, transformed and saved into flux_vec[2], will be
   * overwritten) and need to convert it to dz/dt for calculation.
   * Use chain rule: dP/dt = dP/dZ * dZ/dz * dz/dt
   * dP/dZ = d ( P0 * exp(-Z/H0) ) / dZ = -1/H0 * P(Z)
   * dZ/dz = d ( (1-z)*Z_min + z*Z_max ) / dz = Z_max - Z_min
   * 
   * => dz/dt = dP/dt * 1 / (dP/dZ * dZ/dz)
   *          = dP/dt   * (-H0) / (P * (Z_max-Z_min))
   *    [1/h] = [hPa/h] * [km] / ([hPa] * [km])
   */
  ...
  flux_vec[2] = flux_vec[2] * (-(H0 / (pressure * height_of_atmosphere)));
...
/* 2D Interpolation for ground pressure. Currently deactivated, may be useful later. */
  ...
  intpol_met_time_2d (mptrac_context->mptrac_meteo1, mptrac_context->mptrac_meteo1->us,
                      mptrac_context->mptrac_meteo2, mptrac_context->mptrac_meteo2->us, physical_time_s, lon, lat, flux_vec, ci, cw, 1);
  intpol_met_time_2d (mptrac_context->mptrac_meteo1, mptrac_context->mptrac_meteo1->vs,
                      mptrac_context->mptrac_meteo2, mptrac_context->mptrac_meteo2->vs, physical_time_s, lon, lat, flux_vec + 1, ci, cw, 1);
...
t8dg_mptrac_box_indicator_fn (const double x[3], const double t, void *fn_data)
{
  ...
  if (vert_dist_from_center < radius_vert_km && horiz_dist_from_center < radius_horiz_deg) {
    return 1;
  }
  ...
  /* transform to [0,1] */
  horiz_dist_from_center =
    smoothing_factor_horiz == 0 ? 0 : (horiz_dist_from_center - radius_horiz_deg) / (radius_horiz_deg * smoothing_factor_horiz);
  vert_dist_from_center =
    smoothing_factor_vert == 0 ? 0 : (vert_dist_from_center - radius_vert_km) / (radius_vert_km * smoothing_factor_vert);
